...
- Safest current behavior: Keep every watch flag false until the target-bound privacy evidence, application-only credential proof, fresh-D1 staging smoke, and separate release approval exist. When enabled, expose only read-only registration, polling, candidate display, and X review links. A future X-write milestone must require a separate named-human review and must never let Cron or an agent perform a Like or Repost.
