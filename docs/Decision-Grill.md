...
- Safest current behavior: Retain only the least-privilege `XInteractionWatchReadAdapter`, the separate `XInteractionWatchPublishingAdapter`, opaque identifiers, and ADR-012. Provide no runtime route, Cron, database migration, UI action, or X write. A future implementation may detect candidates automatically but must require a separate named-human click for every exact target and must never let Cron or an agent perform the Like or Repost.
- Needed answer: Explicitly advance the active milestone to a default-disabled interaction-watch capability under ADR-012. Production enablement remains a later, separate release approval and must pass a fresh X policy review.
- Resolution: On 2026-07-24 the user explicitly advanced the default-disabled monitoring and candidate-display milestone. Migration 029 enforces one immutable verified target and append-preserved ID-only candidates. The Worker verifies the username through a read-only adapter, detects only original posts, deduplicates by post ID, advances a durable cursor, and exposes registration, manual poll, and candidate-list routes. Cron receives only `XInteractionWatchReadAdapter`; the UI provides only an X review link. Runtime X reads use a dedicated application-only Bearer Token rather than any User Context write credential. Activation requires an auditable privacy-review reference, is mutually exclusive with every X-write release, and requires a fresh-D1 staging smoke proving reply/Repost exclusion, exact-once discovery, no body column, and no X-write audit. Dedicated staging-smoke and release gates remain false in committed staging and production configuration. No Like, Repost, reply, DM, follow, approval, or X-write route was added.
