...
- Resolution: On 2026-07-24 the user explicitly advanced the default-disabled monitoring and candidate-display milestone. Migration 029 enforces one immutable verified target, a 15-minute/96-per-UTC-day poll policy, and append-preserved ID-only candidates. The Worker verifies the username through a read-only adapter, requires the resolved numeric user ID to match the privacy-reviewed target, rejects any credential that authenticates `/2/users/me` as User Context, detects only original posts, deduplicates by post ID, advances a durable cursor, and exposes registration, manual poll, and candidate-list routes. In a watch release the scheduled handler takes the watch-only branch and does not initialize or invoke the publication writer; the poller receives only `XInteractionWatchReadAdapter`. The UI provides only an X review link. Activation requires an auditable target-bound privacy-review reference, is mutually exclusive with every X-write release, and requires a fresh-D1 staging smoke proving reply/Repost exclusion, exact-once discovery, no body column, and no X-write audit. Dedicated staging-smoke and release gates remain false in committed staging and production configuration. No Like, Repost, reply, DM, follow, approval, or X-write route was added.
