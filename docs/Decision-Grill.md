...
## DG-032 — One-account interaction watch milestone

- Status: RESOLVED
- Evidence: On 2026-07-24 the user approved a flow that detects original posts from one specified X account, queues candidates, and lets a named operator approve one Like plus one Repost. Repository rules keep the active milestone at Phase 1 and permit later behavior only as interfaces or documentation. X's Automation Rules, updated April 2026, prohibit automated Likes.
- Conflict/gap: The operating flow is clear, but approving the flow does not by itself change the repository's active implementation milestone or authorize an automated Like. A previous executable implementation was removed after standards review found direct X client calls and Phase 1 runtime paths.
- Impact: Worker routes, scheduled polling, D1 persistence, operator UI, X User Context scopes, rate limits, partial-outcome reconciliation, staging evidence, production release gates, and policy compliance.
- Safest current behavior: Keep every watch flag false until the target-bound privacy evidence, application-only credential proof, newly migrated empty-watch-table staging smoke, and separate release approval exist. When enabled, expose only read-only registration, polling, candidate display, and X review links. A future X-write milestone must require a separate named-human review and must never let Cron or an agent perform a Like or Repost.
- Needed answer: None for implementation. Production enablement remains a later, separate release approval and must pass a fresh X policy review.
- Resolution: On 2026-07-24 the user explicitly advanced the default-disabled monitoring and candidate-display milestone. Migration 029 enforces one immutable verified target, a 15-minute/96-per-UTC-day poll policy, and append-preserved ID-only candidates. The Worker verifies the username through a read-only adapter, requires the resolved numeric user ID to match the privacy-reviewed target, and accepts only X's structured `Unsupported Authentication` response from `/2/users/me` as proof of an application-only credential. It detects only original posts, deduplicates by post ID, advances a durable cursor, and exposes registration, manual poll, and candidate-list routes. In a watch release the scheduled handler takes the watch-only branch and does not initialize or invoke the publication writer; conflicting or incomplete watch configuration disables both watch and X-write processing and appends a fail-closed audit. The poller receives only `XInteractionWatchReadAdapter`. The UI provides only an X review link. Activation requires an auditable target-bound privacy-review reference, is mutually exclusive with every X-write release, and requires a newly migrated empty-watch-table staging smoke proving reply/Repost exclusion, exact-once discovery, no body column, and no X-write audit after watch registration. The dedicated staging smoke passed, and production was subsequently activated for one explicitly selected, privacy-reviewed target under `privacy_review_watch_20260724_v1` and the user's separate production instruction. The selected username was verified through X and bound to its resolved numeric ID as a Worker Secret rather than a committed variable. X's official Automation Rules were freshly reviewed on 2026-07-24; the release remains read-only, automated Likes remain prohibited, and no automated Repost capability was enabled. Phase 3 publication and named-human X-write releases are disabled; no Like, Repost, reply, DM, follow, approval, or X-write route was added.
