...
既定無効の追加マイルストーンとして、名前付き人間が登録した1件のX
ユーザー名をAPIで数値IDへ照合し、そのユーザーの新規オリジナル投稿を
定期的に検知して候補一覧へ表示できる。

- Xへのアクセスは読み取り専用Adapterを経由する。
- 運用資格は専用の読み取り専用Bearer Tokenとし、投稿可能なUser Context資格を
  Cronへ渡さない。
- 監視対象は同時に1件だけとし、返信とリポストを候補から除外する。
- 保存するのは投稿ID、投稿者ID、時刻、監査情報だけで、本文は保存しない。
- 緊急停止中、専用機能フラグ無効時、staging検証またはrelease承認不足時は
  検知・登録・候補作成を拒否する。
- 対象アカウントのprivacy review証跡IDがない場合は登録・検知を拒否し、
  証跡の対象XユーザーIDと照合後のIDが一致しない場合も拒否する。
  証跡IDを本文や個人情報を含めず監査へ記録する。
- Bearer Tokenは`/2/users/me`でUser Contextとして認証できないことを
  登録時と検知時に確認し、判定不能な場合も拒否する。
...
- 候補画面はX上の対象投稿を人間が確認するリンクだけを提供し、
  いいね、リポスト、返信、DM、フォローを実行しない。
- Xへの各操作はこのマイルストーンの対象外であり、別の仕様変更と承認を
  必要とする。
